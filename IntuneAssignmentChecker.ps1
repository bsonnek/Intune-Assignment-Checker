    ...
    switch ($selection) {
        ...
        '7' {
            ...
            try {
                ...
                # Use a fixed Reports directory on C:\ and create it if missing
                $reportDir = "C:\Intune-Assignment-Checker\Reports"
                if (!(Test-Path -Path $reportDir)) { New-Item -ItemType Directory -Path $reportDir | Out-Null }
                $timestamp = Get-Date -Format "yyyyMMdd_HHmmss"
                $filePath = Join-Path $reportDir "IntuneAssignmentReport_$timestamp.html"
